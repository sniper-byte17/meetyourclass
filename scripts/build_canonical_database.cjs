// scripts/build_canonical_database.cjs
const fs = require('fs');

// Read raw user list
const userListText = fs.readFileSync('scripts/user_schools_list.txt', 'utf8');
const userLines = userListText.split('\n').map(l => l.trim()).filter(Boolean);

// Read existing schoolsData.ts
...
// Parse existing schools
const matchArray = parts[0].match(/export const INITIAL_SCHOOLS: School\[\] = (\[[\s\S]*?\]);/);
const existingSchools = eval(matchArray[1]);
console.log(`Read ${existingSchools.length} currently loaded schools.`);

// Clean raw handle
function formatHandle(rawHandle) {
  if (!rawHandle) return 'no username';
  let h = rawHandle.trim();
  if (h.toLowerCase().includes('no username') || h === '') return 'no username';
  h = h.replace(/\(.*\)/g, '').replace(/\*/g, '').trim();
  if (!h) return 'no username';
  return h.startsWith('@') ? h : '@' + h;
}

// Clean normalize key for user raw strings
function toKey(str) {
...
// Aliases mapping user raw strings or toKey to canonical key
const ALIASES = {
  'georgewashngton': 'george washington',
  'syracruse': 'syracuse',
  'southerncalifornia': 'usc',
  'texaschristian': 'tcu',
  'queenscharlotte': 'queens charlotte',
  'notredame': 'notre dame',
  'westcarolina': 'western carolina',
  'pennyslvania': 'upenn',
  'iowa': 'uiowa',
  'minnesotaduluthumd': 'umd duluth',
  'loyolachicago': 'loyola chicago',
  'missisipi': 'mississippi state',
  'mississipi': 'mississippi state',
  'newyork': 'nyu',
  'johncaroll': 'john carroll',
  'northeastern': 'northeastern',
  'miami': 'miami ohio',
  'wakeforest': 'wake forest',
  'northcarolinaat': 'nc a&t',
  'chicago': 'uchicago',
  'sanfrancisco': 'usfca',
  'tennesse': 'tennessee',
  'charleston': 'cofc',
  ...
  'brighamyoung': 'byu',
  'amhrest': 'amherst',
  'worchester': 'worcester state',
  'californialongbeach': 'cal state long beach',
  'californiaeastbay': 'cal state east bay',
  'californiachico': 'cal state chico',
  'californiafortcollins': 'colorado state',
  'westvirginia': 'wvu',
  'chrisophernewport': 'christopher newport',
  'virginiacommonwealth': 'vcu',
  'eastmichigan': 'eastern michigan',
  'utchattanoga': 'ut chattanooga',
  'northcarolinacentral': 'nc central',
  'uofiowa': 'uiowa',
  'uofwashington': 'washington',
...
// 1. Process user list entries
// Map canonical key -> chosen handle
const canonicalHandles = new Map();
const userEntries = [];

for (const line of userLines) {
  const [name, handleRaw] = line.split('|').map(s => s.trim());
  const handle = formatHandle(handleRaw);
  const normKey = toKey(name);
  const targetMetaKey = ALIASES[normKey] || Object.keys(METADATA).find(k => k === normKey || toKey(k) === normKey) || normKey;

  if (!canonicalHandles.has(targetMetaKey)) {
    canonicalHandles.set(targetMetaKey, handle);
  } else {
    // If previous was 'no username' and this one has actual handle, upgrade
    const prev = canonicalHandles.get(targetMetaKey);
    if (prev === 'no username' && handle !== 'no username') {
      canonicalHandles.set(targetMetaKey, handle);
    }
  }
}

console.log(`Unique canonical target keys mapped from user list: ${canonicalHandles.size}`);

// Verify how many have real handles
let realHandlesCount = 0;
for (const [k, h] of canonicalHandles.entries()) {
  if (h !== 'no username') realHandlesCount++;
}
console.log(`Total canonical schools with actual IG handles: ${realHandlesCount}`);
...
for (const school of existingSchools) {
  const normName = toKey(school.name);
  const normShort = toKey(school.shortName);
  
  // Find key in canonicalHandles
  let matchedKey = null;
  for (const [cKey, handle] of canonicalHandles.entries()) {
    const meta = METADATA[cKey];
    if (cKey === normName || cKey === normShort) {
      matchedKey = cKey;
      break;
    }
    if (meta && (toKey(meta.name) === normName || toKey(meta.shortName) === normShort)) {
      matchedKey = cKey;
      break;
    }
  }
  ...
  if (matchedKey) {
    matchedMetaKeys.add(matchedKey);
    const chosenHandle = canonicalHandles.get(matchedKey);
    // Only update if not 'no username' OR if current handle is missing
    if (chosenHandle !== 'no username' || !school.instagramHandle) {
      school.instagramHandle = chosenHandle;
    }
  }
  updatedSchools.push(school);
}

// 3. Add any universities in canonicalHandles / METADATA not yet in updatedSchools
const existingSchoolNames = new Set(updatedSchools.map(s => toKey(s.name)));

for (const [cKey, handle] of canonicalHandles.entries()) {
  const meta = METADATA[cKey];
  if (!meta) continue;
  if (existingSchoolNames.has(toKey(meta.name))) continue;

  const slug = cKey.replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
  const newSchool = {
    id: slug,
    slug: slug,
    name: meta.name,
    ...
    memberCount: Math.floor(450 + Math.random() * 1200),
    established: meta.est,
    instagramHandle: handle
  };

  updatedSchools.push(newSchool);
  existingSchoolNames.add(toKey(meta.name));
}

console.log(`Total final schools in database: ${updatedSchools.length}`);
const finalWithHandles = updatedSchools.filter(s => s.instagramHandle && s.instagramHandle !== 'no username').length;
console.log(`Final schools with IG handles: ${finalWithHandles}`);

// Write back to schoolsData.ts
const formattedSchools = updatedSchools.map(s => {
  return `  {
